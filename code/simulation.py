...
if __name__ == '__main__':
    ...
    print("START: %02d/%02d/%02d: %02d:%02d:%02d" % (t.tm_mon, t.tm_mday, t.tm_year, t.tm_hour, t.tm_min, t.tm_sec), file=results)
    ...
    state = [75, 0, 1, 0, 0]
    for i in range(0,1801,30):
        print("%d seconds" % i)
        state[1] = i
        p_w = prob(state)
        print("P_w = %.3f" % p_w)
    print("P_w = %.3f" % p_w, file=results)
    ...
    t = time.localtime()
    print("FINISH: %02d/%02d/%02d: %02d:%02d:%02d" % (t.tm_mon, t.tm_mday, t.tm_year, t.tm_hour, t.tm_min, t.tm_sec), file=results)
